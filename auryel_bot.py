...
def init_db():
    ...
    c.execute("""
        CREATE TABLE IF NOT EXISTS users (
            ...
            relance_abonne_envoyee BOOLEAN DEFAULT FALSE,
...
def get_user(phone):
    ...
    c.execute("""SELECT phone,email,prenom,guide,nom_affiche,nb_echanges,dernier_outil,
        ...
        stripe_customer_id,relance_j6_envoyee,relance_j8_envoyee,relance_abonne_envoyee,
    ...
    if row:
        return {
            ...
            "relance_j8_envoyee":row[14],"relance_abonne_envoyee":row[15],
            "dernier_rituel_date":row[16],"dernier_rituel_type":row[17],
            "depuis_site":row[18]
        }
    return None
...
def detecter_prenom(message):
    """Détecte un prénom en excluant les mots communs"""
...
def msg_fin_conv(nom):
    return f"Vas-y... je serai là quand tu reviens 🌙"
...
def stripe_webhook():
    ...
    if event["type"] == "checkout.session.completed":
        s = event["data"]["object"]
        phone = s.get("client_reference_id") or s.get("metadata",{}).get("phone")
        if phone:
            user = get_user(phone)
            update_user(phone, abonne=True, etat="normal",
            ...
            def send_retour(num, u):
                ...
                send_message(num, msg_retour_paiement(nom, prenom))
                add_message(num, "assistant", msg_retour_paiement(nom, prenom))
        ...
        if customer_id:
            conn = get_conn()
            c = conn.cursor()
            c.execute("SELECT phone FROM users WHERE stripe_customer_id=%s", (customer_id,))
            row = c.fetchone()
            conn.close()
            if row:
                update_user(row[0], abonne=False, etat="pause")
...
def cron_daily():
    ...
    for phone in phones:
        ...
        # Utilisateur abonné → relance J+3 d'absence
        if user["abonne"]:
            absence = get_jours_absence(phone)
            if absence >= 3 and not user["relance_abonne_envoyee"]:
                send_message(phone, msg_relance_abonne(nom, prenom))
                add_message(phone, "assistant", msg_relance_abonne(nom, prenom))
                update_user(phone, relance_abonne_envoyee=True)
                relances_abonnes += 1
                time.sleep(1)
            elif absence < 1:
                # Reset le flag si l'utilisateur a réécrit
                update_user(phone, relance_abonne_envoyee=False)
            continue

        # Non abonné → séquence conversion
        ...
        if nb_jours == 6 and not user["relance_j6_envoyee"]:
            ...
            update_user(phone, relance_j6_envoyee=True, etat="attente_paiement")
            j6 += 1; time.sleep(1)
            ...
            update_user(phone, relance_j8_envoyee=True)
            j8 += 1; time.sleep(1)
...
async function openConv(p){{
  cp=p;document.getElementById('modal').classList.add('open');
  document.getElementById('messages').innerHTML='<div style="text-align:center;padding:20px;color:#8a7a6a">Chargement...</div>';
  try{{
    const r=await fetch('/admin/conversation?phone='+encodeURIComponent(p));
    const d=await r.json();
    document.getElementById('modalTitle').textContent=(d.prenom||'')+(d.nom_affiche?' — '+d.nom_affiche:'');
    const m=document.getElementById('messages');m.innerHTML='';
    if(!d.messages||d.messages.length===0){{m.innerHTML='<div style="text-align:center;padding:20px;color:#8a7a6a">Aucun message</div>';}}
    else{{d.messages.forEach(x=>{{const el=document.createElement('div');el.className='msg '+x.role;const t=x.timestamp?x.timestamp.substring(0,16).replace('T',' '):'';el.innerHTML=x.content.replace(/</g,'&lt;').replace(/>/g,'&gt;')+"<div class='msg-time'>"+t+"</div>";m.appendChild(el);}});m.scrollTop=m.scrollHeight;}}
  }}catch(e){{document.getElementById('messages').innerHTML='<div style="text-align:center;padding:20px;color:#ff6b6b">Erreur</div>';}}
}}
...
async function sendRituel(){{
  const types=['psaume','carte','chiffre'];
  const t=types[Math.floor(Math.random()*3)];
  await fetch('/admin/send-rituel?phone='+encodeURIComponent(cp)+'&type='+t,{{method:'POST'}});
  alert('Rituel '+t+' envoyé ✨');openConv(cp);
}}
async function sendManual(){{
  const msg=document.getElementById('sendInput').value.trim();
  if(!msg||!cp)return;
  const r=await fetch('/admin/send',{{method:'POST',headers:{{'Content-Type':'application/json'}},body:JSON.stringify({{phone:cp,message:msg}})}});
  const d=await r.json();
  if(d.ok){{document.getElementById('sendInput').value='';openConv(cp);}}else{{alert('Erreur');}}
}}
...
def admin_pause():
    if not admin_auth(): return jsonify({"error":"unauthorized"}), 401
    update_user(request.args.get("phone",""), etat="pause")
    return jsonify({"ok":True})

@app.route("/admin/resume", methods=["POST"])
def admin_resume():
    if not admin_auth(): return jsonify({"error":"unauthorized"}), 401
    update_user(request.args.get("phone",""), etat="normal")
    return jsonify({"ok":True})

@app.route("/admin/set-abonne", methods=["POST"])
def admin_set_abonne():
    if not admin_auth(): return jsonify({"error":"unauthorized"}), 401
    update_user(request.args.get("phone",""), abonne=True, etat="normal",
               date_abonnement=datetime.now().isoformat())
    return jsonify({"ok":True})
...
def admin_send_rituel():
    ...
    update_user(phone, dernier_rituel_date=date.today().isoformat(),
